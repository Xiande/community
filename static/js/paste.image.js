  ...
  $.fn.extend({
    pasteImage: function(callback){
      ...
      Chrome.prototype.exec = function(){
        ...
        that.$element.on("paste",function(event){
          var items = (event.clipboardData || event.originalEvent.clipboardData).items,
              //取出html对象
              htmlBlobs = util.getProperty(items, "type", /image\/png/),
              imgElements = [],
              loadedCount = 0,
              htmlImages = [],
              htmlResults = [],
          ...
          for(i = 0;i<htmlBlobs.length;i++){
            ...
            var file = htmlBlobs[i].getAsFile();
      ...
      Chrome.prototype.isSuport = function(){
          return window.navigator.userAgent.toLowerCase().indexOf("chrome") > -1 || window.ActiveXObject;
      };
