...
( function() {
	...
	var previewCmd = { modes: { wysiwyg: 1, source: 1 },
		...
		exec: function( editor ) {
			...
			if ( config.fullPage )
				sHTML = editor.getData().replace( /<head>/, '$&' + baseTag ).replace( /[^>]*(?=<\/title>)/, '$& &mdash; ' + editor.lang.selfpreview.preview );
			else {
				var bodyHtml = '<body ',
				...
				sHTML = editor.config.docType + '<html dir="' + editor.config.contentsLangDirection + '">' +
					'<head>' +
						baseTag +
						'<title>' + editor.lang.selfpreview.preview + '</title>' +
						//CKEDITOR.tools.buildStyleHtml( editor.config.contentsCss ) +
						CKEDITOR.tools.buildStyleHtml( CCConfig.L_Menu_BaseUrl + "/cc/css/style.default.css" ) +
						CKEDITOR.tools.buildStyleHtml( CCConfig.L_Menu_BaseUrl + "/cc/css/core.css" ) +
					'</head>' + bodyHtml +
					"<div class='index-con mc'>" +
							        "<div class='lis-lef' style='width:780px;'>" +
							            "<h3 class=' lislef-tit mb0'>" + editor.lang.selfpreview.preview + "</h3>" +
							            "<div class='dtl'>" +
							                "<div class='dtl-qus clearfix mb20 '>" +
							                    "<div class='dtl-dis h5 ques-ext'>" +
													editor.getData() +
						 						"</div>" +
							                "</div>" +
							            "</div>" +
			...
			if ( CKEDITOR.env.ie ) {
				window._cke_htmlToLoad = sHTML;//eventData.dataValue;
			...
			var oWindow = window.open( sOpenUrl, "", 'toolbar=yes,location=no,status=yes,menubar=yes,scrollbars=yes,resizable=yes,width=' +
	...
	CKEDITOR.plugins.add( pluginName, {
		...
		icons: 'selfpreview,selfpreview-rtl', // %REMOVE_LINE_CORE%
		...
		init: function( editor ) {
			...
			editor.ui.addButton && editor.ui.addButton( 'selfpreview', {
				label: editor.lang.selfpreview.preview,
				command: pluginName
			} );
		}
	} );
